...
class TestBacktest(TestCase):
    ...
    def test_assertions(self):
        class Assertive(Strategy):
            def init(self):
                ...
                self.transpose_invalid = self.Indicator(lambda: np.column_stack((self.data.Open,
                                                                                 self.data.Close)))
            ...
            def next(self, FIVE_DAYS=pd.Timedelta('3 days')):
                ...
                if crossover(self.sma, self.data.Close):
                    ...
                    self.open_short(size=.21, limit=price, stop=price, sl=sl, tp=tp)
    ...
    def test_broker_params(self):
        bt = Backtest(GOOG.iloc[:100], SmaCross,
                      cash=1000, commission=.01, margin_rate=.1, trade_on_close=True)
        bt.run()
    ...
    def test_trade_enter_hit_sl_on_same_day(self):
        ...
        class S(S):
            def next(self):
                if self.data.index[-1] == the_day:
                    self.open_long(stop=758, sl=720)
        ...
        class S(Strategy):
            ...
            def next(self):
                if self.data.index[-1] == pd.Timestamp("2004-09-09 00:00:00"):
                    self.open_long(stop=104, sl=103, tp=110)
...
class TestStrategy(TestCase):
    ...
    def test_broker_hedging(self):
        def coroutine(self):
            yield self.open_long(size=2)

            assert len(self.trades) == 1
            yield self.open_short(size=1)
...
class TestOptimize(TestCase):
    ...
    def test_nowrite_df(self):
        ...
        bt = Backtest(df, S)
        _ = bt.run()
        assert values == bt.data.values.ctypes.data
...
class TestPlot(TestCase):
        ...
        class S(Strategy):
            ...
            def next(self):
                ...
                if date == pd.Timestamp('Thu 19 Oct 2006'):
                    self.open_long(stop=484, limit=466, size=100)
                elif date == pd.Timestamp('Thu 30 Oct 2007'):
                    self.position.close()
                elif date == pd.Timestamp('Tue 11 Nov 2008'):
                    self.open_short(stop=self.data.Low,
                                    limit=324.90,  # High from 14 Nov
                                    size=200)

        bt = Backtest(GOOG, S, margin_rate=.1)
        stats = bt.run()
    ...
    def test_indicator_scatter(self):
        ...
        bt.run()
        with _tempfile() as f:
            bt.plot(filename=f, plot_drawdown=False, plot_equity=False, plot_pl=False, plot_volume=False, open_browser=False)
...
class TestLib(TestCase):
    ...
    def test_resample_apply(self):
        ...
        self.assertEqual(res.iloc[-48:].unique().tolist(), [1.2426429999999997, 1.2423809999999995, 1.2422749999999998])
...
@unittest.skipUnless(os.path.isdir(os.path.join(os.path.dirname(__file__), '..', '..', 'doc')), "docs dir doesn't exist")
class TestDocs(TestCase):
    def test_examples(self):
        examples = glob(os.path.join(os.path.dirname(__file__), '..', '..', 'doc', 'examples', '*.py'))
