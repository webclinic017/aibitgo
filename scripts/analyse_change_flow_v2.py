        ...
        for symbol in self.symbols:
            ...
            if not current_holding.get(symbol):
                ...
                if change_pct >= 0 and current_close > 0 and diff_minutes == 0 and current_close > mean_of_24:
                    # 旧版本
                    # if change_pct >= 0 and current_close > 0 and diff_minutes == 0:
            ...
            # 如果持仓了，判断是否要平仓
                ...
                if (change_pct <= 0 or current_close < mean_of_24) and current_close > 0 and diff_minutes == 0:
...
def make_backtest_data(symbols: List[str]) -> pd.DataFrame:
    ...
    for symbol in symbols:
        df = get_local_kline(start_date=start_time, end_date=end_time, symbol_name=symbol.upper() + "BTC",
...
class ChangeFlowBacktest(object):
    def __init__(self):
    ...
    def backtest_enhance_btc_v2(self, symbol_name):
        ...
        # TODO:make a name
        now = datetime.now()
        result_name = f"{symbol_name}_daily"
