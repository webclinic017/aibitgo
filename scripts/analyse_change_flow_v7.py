    ...
    def __call__(self, target):
        ...
        for symbol in self.symbols:
            ...
            # 如果持仓了，判断是否要平仓
                ...
                elif (target.now - buy_time).total_seconds() >= 60 * self.minutes * 5:

                    # 1. 如果说价格回到了 low_15

                    low_15 = target.universe.iloc[-1][f"{self.period}_low_{self.minutes}"]
                    last_current_open = target.universe.iloc[-(2 + self.minutes)][f"{symbol}_Open"]
                    last_current_close = target.universe.iloc[-(2 + self.minutes)][f"{symbol}_Close"]
                    last_last_current_open = target.universe.iloc[-(2 + 2 * self.minutes)][f"{symbol}_Open"]
                    last_last_current_close = target.universe.iloc[-(2 + 2 * self.minutes)][f"{symbol}_Close"]

                    if current_high >= low_15:
                        logger.info(f"平仓{symbol} - {target.universe.iloc[-1].name}")
                        target.rebalance(0, child=f"{symbol}_Open", base=self.base)
                        del current_holding[symbol]

                    # 2. 如果说价格连续两个阳线
                    elif last_current_close > last_current_open and last_last_current_close > last_last_current_open:
                        logger.info(f"平仓{symbol} - {target.universe.iloc[-1].name}")
                        target.rebalance(0, child=f"{symbol}_Open", base=self.base)
                        del current_holding[symbol]
...
class ChangeFlowBacktestV7(object):
    def __init__(self):
    ...
    def backtest_enhance_btc_v7(self, symbol_name):
        logger.info(f"开始回测增强策略 v6 : {symbol_name}")
        period = 99
        minutes = 30
