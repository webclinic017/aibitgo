...
class FourHourAlgo(bt.Algo):
    ...
    def __call__(self, target):
        ...
        for symbol in self.symbols:
            ...
            diff_minutes = int((target.universe.iloc[-1].name.to_pydatetime() - target.universe.iloc[
                -1].name.to_pydatetime().replace(hour=0, minute=0)).total_seconds() / 60) % self.period

            # 如果发现没有持仓，判断是否要开仓
            if not current_holding.get(symbol):

                four_hour_ago_open = target.universe.iloc[-(self.period * (self.open_times + 1))][f"{symbol}_Close"]
                ...
                if change_pct >= 0 and current_close > 0 and diff_minutes == 0 and current_close > mean_of_24:
                    ...
                    # 记录信息
                    target.perm["ADDTIONAL_INFO_DATE_BUY"].append(
                        target.universe.iloc[-(self.period * (self.open_times + 1))].name)
                    target.perm["ADDTIONAL_INFO_PRICE_BUY"].append(four_hour_ago_open)

            # 如果持仓了，判断是否要平仓
            else:
                four_hour_ago_open = target.universe.iloc[-(self.period * (self.close_times + 1))][
                    f"{symbol}_Close"]
                current_close = target.universe.iloc[-2][f"{symbol}_Close"]
                ...
                # chang_pct 要和diff minutes 同时判断
                if change_pct <= 0 and diff_minutes == 0 and current_close > 0:
                    logger.info(f"平仓{symbol} - {target.universe.iloc[-1].name}")

                    target.perm["ADDTIONAL_INFO_DATE"].append(
                        target.universe.iloc[-(self.period * (self.close_times + 1))].name)
...
class ChangeFlowBacktestV3(object):
    def __init__(self):
    ...
    def backtest_enhance_btc_v3(self, symbol_name):
        logger.info(f"开始回测增强策略 v3 : {symbol_name}")
        ...
        # data['24D_Close_mean'] = data.close_day_mean.resample('D').last().rolling(49).mean().asfreq("min")
        data['24D_Close_mean'] = data.close_day_mean.resample('D').max().rolling(49).mean().asfreq("min")
        ...
        for index, value in transactions.iterrows():
            if value.quantity > 0 and not current_info:
                ...
                current_info["盈利"] = (current_info["平仓价格"] - current_info["开仓价格"]) * 100 / current_info["开仓价格"] - 0.25
        ...
        # plt.show()
        logger.info(f"btc回测版本二成功:{symbols}")
