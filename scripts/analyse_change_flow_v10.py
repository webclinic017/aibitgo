    ...
    def __call__(self, target):
        ...
        for symbol in self.symbols:
            ...
            if diff_minutes == 0:
                ...
                day_5_high = max(
                    target.universe.iloc[-(2 + x * self.minutes)][f"high_{self.minutes}"] for x in range(0, 5))
                target.perm["day_5_high"] = day_5_high

            # 如果发现没有持仓，判断是否要开仓
            if not current_holding.get(symbol):

                # if current_close > 0 and diff_minutes == 0 and current_close > high_15:
                ...
                # if current_close > 0 and diff_minutes == 0 and current_close <= low_15 and last_current_close > \
                #         low_15:
                if current_close > 0 and diff_minutes == 0 and current_close <= low_15 and last_current_close > \
                        low_15 and current_close > low_15 * 0.992:
                    logger.info(f"开仓:{symbol}-{target.universe.iloc[-1].name}")
            ...
            # 如果持仓了，判断是否要平仓
                ...
                # 如果说破了low 15
                    ...
                    f"{self.period}_low_{self.minutes}"]:
                    logger.info(f"平仓{symbol} - {target.universe.iloc[-1].name}")
                    target.rebalance(0, child=f"{symbol}_Open", base=self.base)
                    del current_holding[symbol]

                # 如果close大于前五个周期的最高价
                elif day_5_high != 0 and current_high >= day_5_high:
                    logger.info(f"平仓{symbol} - {target.universe.iloc[-1].name} -破了最高价-"
                                f"{target.universe.iloc[-1][f'{symbol}_Open']}")
                    target.rebalance(0, child=f"{symbol}_Open", base=self.base)
                    del current_holding[symbol]
...
class ChangeFlowBacktestV10(object):
    def __init__(self):
    ...
    def backtest_enhance_btc_v10(self, symbol_name):
        logger.info(f"开始回测增强策略 v10 : {symbol_name}")
        period = 49
        minutes = 60
        ...
        data[f"open_{minutes}"] = data.resample(f"{minutes}min")[f"{symbol_name.upper()}-USDT_Open"].first().asfreq(
            "min",
            method="ffill")
        data[f"close_{minutes}"] = data.resample(f"{minutes}min")[f"{symbol_name.upper()}-USDT_Close"].last().asfreq(
            "min",
            method="ffill")
